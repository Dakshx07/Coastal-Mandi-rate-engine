...
export const CatchCalculator: React.FC<CatchCalculatorProps> = ({ isOpen, onClose, summaries }) => {
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-fade-in">
            <div className="bg-white rounded-3xl shadow-2xl w-full max-w-sm overflow-hidden animate-slide-up-spring">
                ...
                <div className="p-6 space-y-5">
                    <div>
                        <label className="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">Select Species</label>
                        <select
                    ...
                    <div>
                        <label className="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">Quantity (kg)</label>
                        <div className="relative">
                            <input
                                ...
                                placeholder="e.g. 50"
                                className="w-full p-3 pl-4 pr-12 bg-slate-50 border border-slate-200 rounded-xl font-bold text-slate-900 focus:ring-2 focus:ring-emerald-500 outline-none transition-all placeholder:text-slate-300 text-lg"
                            />
                    ...
                    {revenue !== null && (
                        <div className="bg-emerald-50 rounded-2xl p-5 border border-emerald-100 animate-fade-in text-center">
                            <div className="text-xs font-bold text-emerald-600 uppercase tracking-wider mb-1">Estimated Revenue</div>
                            <div className="text-3xl font-heading font-extrabold text-emerald-700 flex items-center justify-center">
                                {formatCurrency(revenue)}
                            </div>
                            {selectedSpecies?.change.status === 'UP' && (
                                <div className="mt-2 inline-flex items-center text-xs font-bold text-emerald-600 bg-white px-2 py-1 rounded-lg shadow-sm">
                    ...
                    <button
                        ...
                        className="btn-shine w-full bg-slate-900 text-white py-4 rounded-xl font-heading font-bold shadow-lg shadow-slate-900/20 hover:shadow-xl hover:-translate-y-0.5 transition-all active:scale-[0.98]"
                    >
                        Calculate
                    </button>
                </div>
